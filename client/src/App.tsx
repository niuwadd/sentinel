...
function HomeView({ activeRoom, onRoomSelect }: { activeRoom: string; onRoomSelect: (id: string) => void }) {
  return (
    <main className="flex-grow relative z-10 flex items-center justify-center">
      ...
      <FadeContent blur duration={800} threshold={0} initialOpacity={0} className="absolute top-6 left-6">
        <GlobalStats activeRoom={activeRoom} />
      </FadeContent>

      <FadeContent blur duration={800} delay={200} threshold={0} initialOpacity={0} className="absolute bottom-10 left-0 w-full px-6">
...
function RoomsView({ activeRoom, onRoomSelect }: { activeRoom: string; onRoomSelect: (id: string) => void }) {
  return (
    <main className="flex-grow relative z-10 flex flex-col items-center justify-start pt-20 px-6">
      ...
      <div className="w-full max-w-lg space-y-4">
        {rooms.map((room: { id: string; name: string; temp: number; icon: string; humi: number }) => (
          <button
            key={room.id}
          ...
          >
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${activeRoom === room.id ? "bg-white/20" : "bg-primary-container/20"}`}>
              <span className="material-symbols-outlined" style={activeRoom === room.id ? { fontVariationSettings: "'FILL' 1" } : undefined}>
                {room.icon}
              </span>
            </div>
            <div className="flex-1">
              <p className="font-semibold">{room.name}</p>
              <p className={`text-sm ${activeRoom === room.id ? "text-white/70" : "text-on-surface-variant/70"}`}>
                {room.temp}°C · 湿度 {room.humi}%
              </p>
            </div>
            <span className={`text-lg font-semibold ${activeRoom === room.id ? "text-white" : "text-primary"}`}>{room.temp}°</span>
          </button>
        ))}
      </div>
    </main>
  ...
  return (
    <main className="flex-grow relative z-10 flex flex-col items-center justify-start pt-20 px-6">
      ...
      <div className="w-full max-w-lg grid grid-cols-2 gap-4">
        {[
          { label: "今日用电", value: "12.4 kWh", icon: "bolt", color: "text-primary" },
          { label: "本月电费", value: "¥186", icon: "payments", color: "text-secondary" },
          { label: "CO₂ 减排", value: "3.2 kg", icon: "eco", color: "text-tertiary" },
          { label: "设备效率", value: "92%", icon: "speed", color: "text-primary" },
        ].map((stat) => (
          <div
            key={stat.label}
      ...
      <div className="w-full max-w-lg mt-6 p-4 rounded-2xl bg-[rgba(255,248,246,0.45)] backdrop-blur-[32px] border border-white/40">
        <p className="text-sm font-semibold text-on-surface mb-3">本周温度趋势</p>
        <svg className="w-full h-16" viewBox="0 0 300 50" preserveAspectRatio="none">
          ...
          <path d="M0,40 Q20,30 40,35 T80,20 T120,25 T160,10 T200,18 T240,8 T280,15 T300,12" fill="none" stroke="#9f402d" strokeWidth="2.5" strokeLinecap="round" />
          <path d="M0,40 Q20,30 40,35 T80,20 T120,25 T160,10 T200,18 T240,8 T280,15 T300,12" fill="url(#statsGrad)" stroke="none" />
        </svg>
      </div>
    </main>
  )
      ...
      <div className="w-full max-w-lg space-y-3">
        {[
          ...
          >
            ...
            <span className="text-sm text-on-surface-variant/70">{item.value}</span>
          </div>
        ))}
      </div>
    </main>
...
function App() {
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-background text-foreground overflow-hidden">
      {activeTab === "home" && <HomeView activeRoom={activeRoom} onRoomSelect={setActiveRoom} />}
      {activeTab === "rooms" && <RoomsView activeRoom={activeRoom} onRoomSelect={setActiveRoom} />}
